...
Tested and tuned for the three breakpoints called out in the brief:

- **Desktop (1280px+)** — full layouts: Hero with floating preview on the right, 3-up KPI grid, 4-column footer, inline navbar links
- **Tablet (768px)** — feature sections stack into single-column flow, KPI grid switches to 2-up via container query on `.kpi-grid-wrap`, navbar still inline
- **Mobile (375px)** — fully single-column, hamburger nav with overlay menu, fluid type via `clamp()` on `.section-wrap` padding, no horizontal scroll (`body { overflow-x: hidden }` as a guard)

Strategy: **mobile-first** Tailwind utilities (`sm:`, `md:`, `lg:`) for page-level layout + **container queries** on `.kpi-grid-wrap` so the KPI grid responds to its own width, not the viewport's — important because the same grid could be dropped into a narrower column without media-query rewrites.

---

## Accessibility

Not a full WCAG audit, but the basics the brief calls out are in place:

- **Semantic landmarks** — `<nav>`, `<main>`, `<section>`, `<footer>` used throughout; no `<div>` soup
- **Heading hierarchy** — single `<h1>` in Hero, `<h2>` per section, no skipped levels
- **`prefers-reduced-motion`** — honored two ways: `useReducedMotion()` inside motion components, plus a global CSS killswitch in `App.css` that neutralizes `.line-flow` / `.floaty` ambient loops and clamps all `animation-duration` / `transition-duration` to `0.001ms`
- **Keyboard navigation** — every interactive element (buttons, links, navbar items, mobile menu toggle, CTA buttons, retry button) is reachable via Tab and operable via Enter/Space
- **Focus rings** — global `:focus-visible` rule in `App.css` paints a 2px `accentPrimary` outline with 2px offset on every focused element
- **Decorative SVGs** — `aria-hidden="true"` on ornamental graphics (sparklines, particles, gradient orbs) so screen readers skip them
- **Labeled controls** — icon-only buttons and provider logos carry `aria-label`s; the mobile menu button toggles `aria-expanded`
- **Color contrast** — body text uses `--color-text-primary` (#0E0F11) on `--color-bg-primary` (#FFFFFF) which passes WCAG AA at all sizes; accent text is used only on large headings / pill backgrounds where contrast is sufficient
